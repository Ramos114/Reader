...
<div class="bg-light lter b-b wrapper-md">
  <h1 class="m-n font-thin h3">修改普通管理员</h1>
  <label id="ga_id" hidden="hidden">${ga_info.id }</label>
  <label id="createDate" hidden="hidden">${ga_info.createDate }</label>
  <label id="createName" hidden="hidden">${ga_info.createName }</label>
  <label id="optLock" hidden="hidden">${ga_info.optLock }</label>
      ...
      <div class="panel-body">
        <div class="form-horizontal ng-pristine ng-valid ng-valid-date ng-valid-required ng-valid-parse ng-valid-date-disabled">
          <div class="form-group">
            ...
            <div class="col-sm-4">
              <input type="text" id="userName" class="form-control" placeholder="6-16位，由字母、数字、字符组成" value="${ga_info.userName }" disabled="disabled"> 
          ...
          <input id="roleId" value="${ga_info.role.id }" type="hidden">
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-3">
              <input type="text" id="realName" class="form-control" placeholder="由1-6个中文字符组成" value="${ga_info.realName }">    
          ...
          <div class="form-group">
            ...
            <div class="col-sm-4">
              <input type="text" id="idCard" class="form-control" placeholder="请输入身份证号码，为15-18位数字" value="${ga_info.idCard }">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-4">
              <input type="text" id="telNumber" class="form-control" placeholder="请输入正确 的手机号码" value="${ga_info.telNumber }">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-4">
              <input type="text" id="email" class="form-control" placeholder="请输入正确的邮箱格式，例如zhangsan@163.com" value="${ga_info.email }">
          ...
          <div class="col-sm-4">
            <select id="status" class="form-control m-b">
             <c:if test="${ga_info.status=='可用'}">
              <option value="可用" selected="selected">可用</option>
              <option value="不可用">不可用</option>
              </c:if>
              <c:if test="${ga_info.status=='不可用'}">
          ...
          <div class="line line-dashed b-b line-lg pull-in"></div>
            ...
            <div class="col-sm-4 col-sm-offset-2">
              ...
              <button type="submit" class="btn btn-primary" id="updateGA">保存</button>
          </div>
        </div>
        </div>
      </div>
...
$(document).ready(function(){
	...
	var id=$("#ga_id").html();
		...
		if(
		   ...
		   !$("#telNumber_check").hasClass("text-danger glyphicon glyphicon-remove formCheck")&&
		  ...
		  // alert(userName+"##"+loginPassword+"##"+realName+"##"+gender+"##"+idCard+"##"+telNumber+"##"+email+status+"##");
			 $.ajax({
				type: 'post',
				url: "GeneralAdminManageAction!updateGA.action",
				data:{
				    "generalAdmin.id":id,		
					"generalAdmin.userName":userName,
					...
					"generalAdmin.createDate":createDate,					
					"generalAdmin.createName":createName,
					"generalAdmin.optLock":optLock,
			    ...
			    success: function(data){
					if (data == "success") {						
						...
						layer.msg("恭喜，修改普通管理员成功！", {icon: 1});
					} else {
						layer.msg("抱歉，修改普通管理员失败！", {icon: 2});
					}
			    },
			    cache:false
			});
		}else{
			layer.msg("请先填写正确的普通管理员信息再进行确认操作！", {icon: 7});
		}
	});
});
</script>