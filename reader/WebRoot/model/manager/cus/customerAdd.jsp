...
<div class="bg-light lter b-b wrapper-md">
  <h1 class="m-n font-thin h3">添加会员</h1>
</div>
      ...
      <div class="panel-body">
        <div class="form-horizontal ng-pristine ng-valid ng-valid-date ng-valid-required ng-valid-parse ng-valid-date-disabled">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-3">
              <input type="text" id="realName" class="form-control" placeholder="由2-10个中文字符组成">    
          ...
          <div class="form-group">
            ...
            <div class="col-sm-4">
              <input type="text" id="idCard" class="form-control" placeholder="请输入身份证号码，为18位数字字符组成">
...
$(document).ready(function(){
	...
	$("#saveCustomer").click(function() {
		...
		var idCard = $("#idCard").val();
		var email = $("#email").val();
		var rankId = $('#level option:checked').val();
		...
		if($("#userName_check").hasClass("text-success glyphicon glyphicon-ok formCheck")&&
		   ...
		   $("#email_check").hasClass("text-success glyphicon glyphicon-ok formCheck")){ //判断所有信息格式都正确
			...
			layer.confirm("是否确认保存？",{icon: 3},function(){
				$.ajax({
					type: 'post',
					url: "CustomerAction!saveCustomer.action",
					data:{
						"customer.userName":userName,
				    ...
				    success: function(data){
						if (data == "success") {						
							...
							layer.msg("恭喜，添加会员成功！！", {icon: 1});
						} else {
							layer.msg("抱歉，添加会员失败！", {icon: 2});
						}
				    },
				    cache:false
				});
