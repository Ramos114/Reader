	...
	<div class="panel panel-default">
		...
		<div class="row wrapper">
			...
			<div class="col-sm-4">
				<div class="input-group">
					<input type="text" id="slcName" class="input-sm form-control"
						placeholder="请输入小类别名称"> 
		...
		<div class="table-responsive">
			<table class="table table-striped b-t b-light">
				<thead>
					<tr>
						<th style="width:15%">小类类别编号</th>
						<th style="width:20%">小类类别名称</th>
						<th style="width:25%">类别说明</th>
						<th style="width:15%">大类类别名称</th>
...
<script type="text/javascript">
	...
	function doRequest(currentPage,bgcName,slcName) {
		$
				.ajax({
					type : 'post',
					url : "${basePath}${pageContext.request.contextPath}/CategoryAction!slcShow.action",
					data : {
						"page" : currentPage,
						"rows" : $("#page_everySize").val(),
						"bgcName" : bgcName,
			...
			$.ajax({
					...
					success : function(status) {
						if (status == "slcdel_success") {
							if ($("#slcName").val() == null
									|| $("#slcName").val().length == 0) {
								if ($("#findbybgcName").find("option:selected")
										.text() == "全部") {
									doRequest(1,null,null);
								} else {
									doRequest(1, $("#findbybgcName")
											.find("option:selected").text(),null);
								}
							} else {
								doRequest(1,null,$("#slcName").val());
