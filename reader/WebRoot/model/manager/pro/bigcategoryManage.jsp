			...
			<table class="table table-striped b-t b-light">
				<thead>
					<tr>
						...
						<th style="width:15%">大类类别编号</th>
						<th style="width:20%">大类类别名称</th>
...
<script type="text/javascript">
	...
	function doRequest(currentPage) {
		$.ajax({
					type : 'post',
					url : "${basePath}${pageContext.request.contextPath}/CategoryAction!bgcShow.action",
					data : {
						"page" : currentPage,
	...
	function setDataTable(data) {
		...
		for (var i = 0; i < data.result.length; i++) {
			...
			if (data.result[i].bgcId == null) {
				data.result[i].bgcId = "";
			}
			if (data.result[i].bgcName == null) {
				data.result[i].bgcName = "";
			}
			if (data.result[i].bgcRcontent == null) {
				data.result[i].bgcRcontent = "";
			}
			if (data.result[i].bgcImgUrl == null) {
				data.result[i].bgcImgUrl = "";
			}
			if (data.result[i].bgccontent == null) {
				data.result[i].bgccontent = "";
			}
		...
		$("#tableTbody")
				.append(
						"<tr>"
								...
								+ data.bgcName
								+ "</td>"
								+ "<td>"+ data.bgccontent+ "</td>"
								+ "<td>"+ data.bgcRcontent+ "</td>"
								+"<td>"+"<a class='thumbnail'>"
								+"<img  src='${basePath }${pageContext.request.contextPath}/"+data.bgcImgUrl+"' onclick='showBigImg(\""+data.bgcImgUrl+"\")'>"
								+"</a>"
								+ "<td> <button class='btn btn-sm btn-icon btn-info' title=\"修改大类\" onclick='toSkit(\"CategoryAction!edit_ui.action?bigcategory.id="+ data.id+ "\");'>"
	...
	function addbgc() {
		toSkit("${basePath }${pageContext.request.contextPath}/model/manager/pro/add_bigcategory.jsp");
	}
